...
// static declarations
// ---------------------------------------------
private final double DISTANCE_TRAVELED_IN_SEC = ((8.0 /* ft */ * 12.0)
        + 6.0);
...
public int getDistanceFromNearestBumper ()
{
    ...
    if (normalizedDistance > this.distanceFromNearestBumper)
        ...
        normalizedDistance = normalizedDistance
                - this.distanceFromNearestBumper;
        }
    // -------------------------------------
    ...
    if (this.printToScreen == true)
        ...
        System.out.println(
                "The distance is " + (normalizedDistance / 12) +
...
private int getDistanceValueWithConfidence ()
{
    ...
    double distanceTraveledPerTic = (this.DISTANCE_TRAVELED_IN_SEC
            / this.sensorPeriodicity) * 2;
    ...
    if ((this.confidenceValueArray[this.presentConfidenceValueIndex] == java.lang.Integer.MAX_VALUE)
        ...
        {
        this.lastGoodValue = this.confidenceValueArray[this.presentConfidenceValueIndex
                - 1];
        // -------------------------------------
        ...
        for (index = 1; (index <= (numberToCheck + 1)) &&
            ...
            if (workConfidenceValueIndex == 0)
                {
                workConfidenceValueIndex = this.confidenceValueArray.length
                        - 1;
                // -------------------------------------
            ...
            if (index == 1)
                ...
                possibleGoodValue = this.confidenceValueArray[workConfidenceValueIndex];
                if (this.confidenceValueArray[workConfidenceValueIndex] < this.weightedDistance)
                    {
                    distanceTraveledPerTic = distanceTraveledPerTic
                            * this.WEIGHT_FACTOR;
                    }
                } // if
            // -------------------------------------
            ...
            if ((workConfidenceValueIndex == 0) &&
                    (Math
                            .abs(this.confidenceValueArray[workConfidenceValueIndex]
                                    -
                                    this.confidenceValueArray[this.confidenceValueArray.length
                                            - 1]) > distanceTraveledPerTic))
                {
                outlier = true;
                }
...
private int setDistanceValueIntoConfidenceArray (int newValue)
{
    ...
    this.presentConfidenceValueIndex = this.presentConfidenceValueIndex
            % this.confidenceValueArray.length;
